...
| NFR-11.1 | 纹理—状态与生命周期风险清单（轨道 B） | - 绑定与槽位污染：任何 `Bind(slot)` 必须在渲染通道结束时恢复激活槽位与绑定状态，防止跨通道串味。验收点联动 AC-NFR5（性能）与 AC-P3/P4（后处理/阴影）。<br>    <br>- 资源所有权与析构时序：`B_Texture` 持有的 `GLuint` 必须由其自身创建与销毁；严禁由外部系统复用或重复 `glDeleteTextures`。在 `Window` 销毁前完成所有纹理与 FBO 的销毁，避免“先毁上下文，后删资源”的未定义行为。<br>    <br>- SRGB/Gamma 一致性：明确 Demo 全局是否启用 sRGB 采样与 `GL_FRAMEBUFFER_SRGB`；纹理格式与后处理链路需保持一致，避免“暗一档/洗白”的视觉偏差。<br>    <br>- Mipmap 与过滤策略：贴图加载时统一策略（是否生成 mipmap、min/mag filter、wrap 模式）。同一类资源必须保持一致策略，避免远景闪烁与近景锯齿。<br>    <br>- 异步或多处持有：禁止将同一 `GLuint` 封装到多个 `B_Texture` 实例；如需共享，必须以“共享同一 `B_Texture` 实例”的方式注入。<br>    <br>- 立方体贴图一致性：CubeMap 的 6 面尺寸、格式、内部格式必须一致；加载失败时必须回退到“占位纹理”，以保证渲染稳定性。<br>    <br>- 纹理单元预算：为后处理与阴影预留固定槽位区间，记录于渲染管线文档，避免与材质采样冲突。<br>    <br>- 资源热加载与重入：若未来引入热加载，必须定义“替换中间态”的屏障（双缓冲或帧边界替换），避免在渲染中途替换 `GLuint`。                                                                                                                                     |
| NFR-11.2 | FBO—完备性与链路一致性（轨道 B）  | - 完备性校验：`B_FrameBuffer` 初始化后与每次尺寸变化后，必须验证 `glCheckFramebufferStatus` 为 `FRAMEBUFFER_COMPLETE`；失败时提供明确错误渠道（日志 + 占位回退路径）。<br>    <br>- 尺寸变更与资源重建：窗口 Resize 时，FBO 的 Color/Depth/Stencil 附件必须成组重建；严禁“只改视口不改附件”的不一致状态。<br>    <br>- 多重采样一致性：若启用 MSAA，明确解析（resolve）流程与目标（直接到屏幕或到中间 LDR 纹理），并在整条后处理链保持位深与色域一致。<br>    <br>- Depth/Stencil 共享策略：禁止多个 FBO 交叉共享同一深度附件；如需共享必须在文档中明确，且通过只读依赖的方式使用。<br>    <br>- 后处理顺序与格式：Bloom/色调映射/锐化等后处理链路的输入输出格式需统一，避免 16F/8U 混用导致 banding 或溢出。<br>    <br>- 调试可观测性：提供“FBO 调试显示”路径（将附件以全屏四分格显示到屏幕），作为回归测试手段。<br>    <br>- 清理与回退：在 FBO 创建失败或附件缺失时，必须回退到“直接渲染至默认帧缓冲”的安全路径，保证 Demo 不崩溃。                                                                                                                                                                                                                                                                                                     |
...
- **轨道 B (`NCLGL_Impl`):**
    ...
    - `B_Texture` 将包装由 `SOIL_load_OGL_texture`（`nclgl` 库所依赖）返回的 `GLuint`。
...
namespace Engine::IAL {
    ...
    // 接口：纹理 (P-0, P-8)
    // nclgl 库没有 I_Texture 对应的类，纹理加载直接返回 GLuint (通过 SOIL)
    class I_Texture {
    public:
        virtual ~I_Texture() {}
...
- 槽位语义：`Bind(slot)` 只负责“在指定槽位绑定本纹理”，不隐式更改其他槽位；实现层必须在渲染通道末尾恢复先前激活槽位。
    
- 只读 ID 暴露：`GetID()` 仅用于与外部 API 交互（调试/调优），不得用于跨对象的生命周期管理或重复删除。
    
- 策略一元化：加载阶段统一设置 wrap/filter/mipmap；运行期不允许随意变更同一资源的采样策略。
    
- CubeMap 约束：当类型为立方体贴图时，`Bind(slot)` 的行为与 2D 纹理一致，但必须保证 6 面一致性；契约层不暴露“单面级别”的写接口。
    

#### I_FrameBuffer 契约补充

- 强制 `IsComplete()`：接口层提供只读完备性查询；任何渲染前置步骤都必须在完备通过后才能执行。
    
- 尺寸与重建：接口层提供只读尺寸查询；尺寸变化由实现层内部完成“附件重建 + 状态校验”。
    
- 附件暴露边界：只允许只读方式获取 `GetColorTexture()`/`GetDepthTexture()`；禁止外部直接替换附件，以避免破坏内部一致性。
...
- **轨道 B (`NCLGL_Impl`):** `B_Factory` 必须实现所有接口。
	...
	- `LoadMesh` / `LoadAnimatedMesh`: **必须** `#include "nclgl/Extra/GLTFLoader.h"`。**必须**在内部调用 `GLTFLoader::Load(path, scene)` 来填充一个 `GLTFScene` 对象。随后，从 `scene.meshes[0]`（网格）和（如果适用）`scene.animations[0]`（动画） 中提取 `nclgl` 指针，并将它们包装在 `B_Mesh` 或 `B_AnimatedMesh` 适配器中返回。
        
    - `LoadTexture`: 包装 `nclgl` 库使用的 `SOIL_load_OGL_texture(...)`。
	    
    - `LoadHeightmap`: **自研** RAW 文件加载器。**必须**在 `B_Factory.cpp` 内部定义一个继承自 `nclgl::Mesh` 的辅助类，在该辅助类的构造函数中访问 `protected` 成员（`vertices` 等）并调用 `BufferData()`，最后 `new` 此辅助类并返回 `nclgl::Mesh*`。
        
    - `CreateQuad`: `nclgl::Mesh` **不**提供 `GenerateQuad`。**必须**采用与 `LoadHeightmap` 相同的策略：定义一个继承自 `nclgl::Mesh` 的内部辅助类，在其构造函数中填充全屏四边形的顶点数据并调用 `BufferData()`。
...
- **交付物:** 一个可以渲染基础立方体（`cube.msh`）的、完全基于 `IAL` 接口的 `Demo` 框架。
...
- **每日任务:**
    ...
    - **Day 5: (高风险) FBO (P-3/P-4) 框架 (NFR-11.3)**
        ...
        - **任务:** `B_FrameBuffer` **必须**依赖 `IAL/I_Texture` 接口，并能正确生成包装了 `GLuint` 的 `B_Texture` 实例作为颜色和深度附件。
    ...
    - **Day 7: 架构验证 (Hello Cube)**
        ...
        - **任务 (轨道 B):**
            ...
            - `B_Factory`: 实现 `LoadMesh` (包装 `nclgl::Mesh::LoadFromMeshFile`，加载 `cube.msh`)。
...
- **每日任务:**
    ...
    - **Day 9: (接口) IAL (Texture) & IAL (Heightmap) (P-0)**
        ...
        - **任务 (轨道 B):**
            ...
            - `B_Factory`: 实现 `LoadTexture` (包装 `SOIL_load_OGL_texture`) 并包装在 `B_Texture` 中。
                
            - `B_Factory`: 实现 `LoadHeightmap`。**必须**在 `B_Factory.cpp` 中定义一个**私有辅助类**（例如 `B_HeightmapMesh_NCLGL`）使其**公有继承** `nclgl::Mesh`。**必须**在该辅助类的构造函数中自研 `.raw` 加载器、填充 `protected` 的 `vertices`, `indices`, `textureCoords` 成员，并调用 `protected` 的 `BufferData()` 方法。`LoadHeightmap` 接口将 `new` 这个辅助类，并将其包装在 `B_Heightmap` 适配器中 (NFR-11.3)。
            - 衔接：`B_Factory::LoadTexture` 完成后，提交一份“纹理策略清单”（mipmap、filter、wrap、sRGB 决策）并在渲染器初始化时应用；为后处理与阴影预留固定槽位表，附在文档。
    ...
    - **Day 10: (渲染) 地形渲染 (P-0) 与场景 T1 (P-2)**
        ...
        - **任务:** `Scene_T1_Peace::Init()` **必须**调用 `m_Factory->LoadHeightmap("Assets/Heightmaps/terrain.raw", ...)` 和 `m_Factory->LoadTexture("Assets/Textures/terrain_grass.png")`。
            
        - **任务:** `Scene_T1_Peace` **必须**将返回的 `I_Heightmap` 添加到 `SceneGraph` (P-6)。
            
        - **任务:** 创建 `Assets/Shaders/Shared/terrain.vert/frag` (NFR-6)，用于地形渲染。
            
        - **验收:** 轨道 B 成功加载并渲染带纹理的地形 (P-0)，场景初始化通过 `I_ResourceFactory` 请求 `I_Texture`，并在材质上传阶段仅调用 `I_Texture::Bind(slot)`；完成“通道末尾状态恢复”检查，记录在单元测试清单。
            
    - **Day 11: (渲染) IAL (Skybox) (P-8) 与光照 (P-7)**
        
        - **任务:** 在 `IAL/I_ResourceFactory.h` 中添加 `LoadCubemap` 接口 (P-8)。
            
        - **任务 (轨道 B):** 实现 `B_Factory::LoadCubemap`，**必须**包装 `nclgl` 库提供的 `SOIL_load_OGL_cubemap` 功能。
        ...
        - **验收:** T1 场景 (P-2) 渲染完成，包含地形 (P-0)、天空盒 (P-8) 和 Blinn-Phong 光照 (P-7)，实现 `B_FrameBuffer` 的完备性校验与尺寸重建流程；加入“FBO 调试显示”视图用于 QA。
    ...
    - **Day 13: (交互) IAL (DebugUI) (P-5)**
        ...
        - **验收:** 轨道 B 正常运行（无 UI）。`Demo` 层的 UI 调用被空实现正确处理，不导致崩溃，进行窗口 Resize、切换全屏、切换后处理开关的稳定性与性能回归；记录 sRGB 开/关两组结果并决定最终策略。
...
`-- src/
    ...
    |       |-- B_Heightmap.h/.cpp      (已实现, 自研 RAW 加载器 + 包装 Mesh)
...
- **每日任务:**
    ...
    - **Day 16: (高级) 高级特性 1 (阴影) (P-4)**
        ...
        - **验收:** 场景中的物体（例如 `ruins.msh`）可将实时动态阴影 (P-4) 投射到地形上。
...
|   `-- nclgl/
|       |-- Meshes/
|       |   |-- character.anm     (P-4 动画)
|       |   |-- character.msh     (P-4 模型)
|       |   |-- cube.msh
|       |   `-- ruins.msh         (P-2 模型)
|       |-- (nclgl 库文件) ...
`-- src/
...
此架构承认两个必要的耦合点（NFR-4），它们是架构切换所必需的：

1. **服务注入 (NFR-9):** `main.cpp` 依赖 `Implementations/` 来实例化 `Factory` 和 `WindowSystem`。
    
2. **资源选择 (NFR-4):** `Game/Scenes/` 依赖预处理器宏来选择特定轨道（`.msh` vs `.gltf`）的资源路径。
...
| |                                                | |                                                       | |
| | #include "IAL/I_WindowSystem.h"                | | #ifdef NCL_USE_CUSTOM_IMPL                            | |
| | #include "IAL/I_ResourceFactory.h"             | |     m_Factory->LoadMesh(".../ruins.gltf");            | |
| |                                                | | #else                                                 | |
| | #ifdef NCL_USE_CUSTOM_IMPL                     | |                                                       | |
| |   #include "Implementations/Custom_Impl/..."   | | #endif                                                | |
| |   windowSystem = make_shared<C_WindowSystem>();| | (依赖 IAL/I_ResourceFactory 接口)                       | |
| | #else                                          | |                                                       | |
| |   #include "Implementations/NCLGL_Impl/..."    | '-------------------------------------------------------' |
| |   windowSystem = make_shared<B_WindowSystem>();| | .-----------------------------------------------------. |
| | #endif                                         | | | Core/Application.cpp (依赖 I_WindowSystem, I_Factory) | |
| |                                                | | | Rendering/Renderer.cpp (依赖 I_Factory, I_Shader 等)  | |
| | Application app(windowSystem, ...);            | | | Core/SceneGraph.cpp (P-6) (依赖 I_Mesh, nclgl::Matrix4) | |
...
`-- src/
    ...
    |           |-- B_Heightmap.h/.cpp  // (自研 RAW 加载器, 包装 nclgl::Mesh)
    |           |-- B_Mesh.h/.cpp       // (包装 nclgl::Mesh)
    |           |-- B_Shader.h/.cpp     // (包装 nclgl::Shader)
    |           `-- B_Texture.h/.cpp    // (包装 SOIL)
    |
...
- 槽位 0–3：材质基础纹理（Albedo/Normal/Roughness/Metallic）。
    
- 槽位 4：阴影贴图（Depth）。
    
- 槽位 5：环境立方体贴图（Skybox/IBL）。
    
- 槽位 6–7：后处理中间纹理（根据链路数量调整）。  
    如需扩展，必须在此表中登记并在渲染器初始化时统一设置采样器状态。
## 8. 项目验收标准
...
| ---------------- | ------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Part A: 核心功能** |                     |                                                                                                                                                                                                                                                                                                                               |
| AC-P0            | 基础场景 (P-0)          | **测试：** 启动 `Scene_T1_Peace`（和平场景）。<br><br>  <br>  <br><br>**检查：** 场景必须包含一个使用 `.raw` 高度图渲染的地形 (P-0)，一个程序化生成的水面 (P-0)，以及一个立方体贴图天空盒 (P-8)。                                                                                                                                                                                       |
| AC-P1            | 场景过渡 (P-1)          | **测试：** 在 T1 场景中，按下 `IAL::KeyCode::T` 键。<br><br>  <br>  <br><br>**检查：** 必须触发一个平滑的过渡 (P-1)，最终显示 T2 场景。                                                                                                                                                                                                                         |
| AC-P2            | 场景识别性 (P-2)         | **测试：** 对比 T1 和 T2 场景。<br><br>  <br>  <br><br>**检查：** T2 场景必须可被清晰地识别为 T1 的“不同时间版本” (P-2)。这必须通过至少三种视觉变化来体现（例如：不同的天空盒、不同的地形纹理、添加/移除/替换了关键模型如 `ruins.msh`）。                                                                                                                                                                      |
...
| AC-NFR-11.TEX    | 纹理状态一致性             | 测试：运行 Demo，开启多通道渲染（主通道 + 后处理 + 阴影）。在 RenderDoc/NSight 中检查任意帧，验证所有通道结束后，活动纹理单元与绑定状态回到预期，且不存在跨通道泄漏。                                                                                                                                                                                                                             |
| AC-NFR-11.GAMMA  | Gamma / sRGB 一致性    | 测试：以 sRGB on/off 两种配置渲染同一场景，比较灰阶卡与肤色样本；选择更接近期望的配置并写入文档。提交截图与说明。                                                                                                                                                                                                                                                               |
| AC-NFR-11.FBO    | 完备性与回退              | 测试：在窗口 Resize 与切换后处理开关后，FBO 状态始终为 `FRAMEBUFFER_COMPLETE`；任何失败均回退至默认帧缓冲渲染且 Demo 不崩溃。提交日志与“FBO 调试显示”截图。                                                                                                                                                                                                                         |
| AC-NFR-11.MSAA   | 解析一致性               | 测试：若在启用 MSAA 时，验证解析到屏幕或到中间纹理后的视觉一致性（边缘锯齿、亮度、色带）；记录位深与格式选择。                                                                                                                                                                                                                                                                    |
| AC-NFR-11.CUBE   | 立方体贴图一致性            | 测试：加载完整 CubeMap 与故意缺面/错尺寸的样本；前者渲染正常，后者触发回退并给出明确日志。                                                                                                                                                                                                                                                                            |

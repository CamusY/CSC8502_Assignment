...
4. **宏驱动的双轨切换:** 架构的切换（轨道 B vs 轨道 C）**必须**通过 C++ 预处理器宏（例如 `#ifdef NCL_USE_CUSTOM_IMPL`）在两个层面完成：
    
    - **服务注入 (DI):** 在依赖注入入口点（`src/CSC8502_Assignment/main.cpp`）切换具体工厂和系统（`B_Factory` vs `C_Factory`）的实例化。
        
    - **资源选择:** 在游戏逻辑层（`src/CSC8502_Assignment/Game/Scenes/`）切换具体资源路径的加载（`"ruins.msh"` vs `"ruins.gltf"`）。
...
| NFR-7  | **职责分离**          | `src/CSC8502_Assignment/` 负责通用的渲染逻辑、应用循环、场景管理，以及**具体的资源路径选择**。<br><br>`Engine/IAL/` 层负责定义“引擎提供的稳定系统 API”，其接口签名**直接使用** `nclgl` 数学类型。<br><br>`nclgl/`, `Engine/Custom/` 和 `Engine/Implementations/` 层负责“具体如何实现 API”。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
- **每日任务:**
    ...
    - **Day 7: 架构验证 (Hello Cube)**
        ...
        - **任务:** `SceneManager` 创建一个默认场景，向 `SceneGraph` (P-6) 添加一个 `SceneNode`，该节点持有 `m_Factory->LoadMesh("third_party/nclgl/Meshes/cube.msh")` 返回的 `I_Mesh`。
...
- **每日任务:**
    ...
    - **Day 14: (核心) 场景 T2 (P-2) 与资源切换 (NFR-4)**
        ...
        - **验收:** `SceneManager` 可以成功切换和渲染 T1、T2 两个场景。轨道 B 成功加载 `ruins.msh`。
    ...
    - **Day 17: (高级) 高级特性 2 (骨骼动画) IAL & B 轨 (P-4)**
        ...
        - **验收:** `IAL` 接口更新。轨道 B 可成功加载 `.msh`/`.anm` 并返回 `B_AnimatedMesh` 实例。
    ...
    - **Day 18: (高级) 高级特性 2 (骨骼动画) 集成 (P-4)**
        ...
        - **任务:** `Scene_T1_Peace` (或 T2) **必须**使用 NFR-4 的宏加载一个动画角色（B 轨加载 `character.msh`/`character.anm`) 并将其添加到 `SceneGraph` (P-6)。
...
| **Part B: 高级功能** |                     |                                                                                                                                                                                                                                                                                                                               |
| AC-P4.1          | 高级：阴影 (P-4)         | **测试：** 观察 T1/T2 场景。<br><br>  <br>  <br><br>**检查：** **必须**实现基于动态方向光（太阳）的实时阴影贴图 (P-4)。至少一个静态物体（例如 `ruins.msh`）和/或一个动态物体（`character.msh`）**必须**能将阴影投射到地形（`Heightmap`）上。                                                                                                                                                         |
